...
for kk=1:prm.freq_len
    ...
    H_l2m_reg = H_lc2d(m_reg_idx,l_reg_idx,kk);
    H_l2m_rnd = H_lc2d(m_rnd_idx,l_rnd_idx,kk);
    H_l2m_gso = H_lc2d(m_gso_idx,l_gso_idx,kk);
    H_l2m_det = H_lc2d(m_det_idx,l_det_idx,kk);
    H_l2m_mi = H_lc2d(m_mi_idx,l_mi_idx,kk);
    H_l2m_fs = H_lc2d(m_fs_idx,l_fs_idx,kk);
    H_l2m_eim = H_lc2d(m_eim_idx,l_eim_idx,kk);
    ...
    H_l2r_reg = H_lc2r(:,l_reg_idx,kk);
    H_l2r_rnd = H_lc2r(:,l_rnd_idx,kk);
    H_l2r_gso = H_lc2r(:,l_gso_idx,kk);
    H_l2r_det = H_lc2r(:,l_det_idx,kk);
    H_l2r_mi = H_lc2r(:,l_mi_idx,kk);
    H_l2r_fs = H_lc2r(:,l_fs_idx,kk);
    H_l2r_eim = H_lc2r(:,l_eim_idx,kk);
    ...
    fprintf('[Cond num] Reg+EIMi: %f, Rand: %f, GSO: %f, Det: %f, MI: %f, FS: %f, EIM: %f\n',cond_db_reg(kk),cond_db_rnd(kk),cond_db_gso(kk),cond_db_det(kk),cond_db_mi(kk),cond_db_fs(kk),cond_db_eim(kk));
    ...
    if ismember(prm.freq(kk), figsave_freq) == 1    
        ...
        boxplot([10*log10(abs(drv_reg(:)).^2),10*log10(abs(drv_rnd(:)).^2),10*log10(abs(drv_gso(:)).^2),10*log10(abs(drv_det(:)).^2),10*log10(abs(drv_mi(:)).^2),10*log10(abs(drv_fs(:)).^2),10*log10(abs(drv_eim(:)).^2)],'Labels',{'Reg+EIM','Rand','GSO','Det','MI','FS','EIM'},'Whisker',1.5);
        ...
        if figsave_flg==1
            ...
            fname{1} = sprintf('%s/fig20a',dir_results);
            fname{2} = sprintf('%s/fig20b',dir_results);
            fname{3} = sprintf('%s/fig20c',dir_results);
            fname{4} = sprintf('%s/fig20d',dir_results);
            fname{5} = sprintf('%s/fig20e',dir_results);
            fname{6} = sprintf('%s/fig20f',dir_results);
            fname{7} = sprintf('%s/fig20g',dir_results);
            fname{8} = sprintf('%s/fig21a',dir_results);
            fname{9} = sprintf('%s/fig21b',dir_results);
            fname{10} = sprintf('%s/fig21c',dir_results);
            fname{11} = sprintf('%s/fig21d',dir_results);
            fname{12} = sprintf('%s/fig21e',dir_results);
            fname{13} = sprintf('%s/fig21f',dir_results);
            fname{14} = sprintf('%s/fig21g',dir_results);
            save_figures([fig(12:18),fig(21:27)], fname, fig_type);
        end
    end
end
...
legend('Reg+EIMi','Rand','GSO','Det','Location','southwest');
...
ylim([-10,80]); 
xlabel('Frequency (Hz)'); ylabel('SDR (dB)');
legend('Reg+EIMi','MI','FS','EIM','Location','southwest');
...
if figsave_flg==1
    ...
    fname{1} = sprintf('%s/fig17a',dir_results);
    fname{2} = sprintf('%s/fig17b',dir_results);
    fname{3} = sprintf('%s/fig17c',dir_results);
    fname{4} = sprintf('%s/fig18a',dir_results);
    fname{5} = sprintf('%s/fig18b',dir_results);
    save_figures(fig(2:6), fname, fig_type);
end
